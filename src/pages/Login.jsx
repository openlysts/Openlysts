...
export default function Login() {
  ...
  // (absolute URLs, protocol-relative //evil.com, backslashes) falls back to
  // /discover — and the URL is rewritten so the unsafe value is never
  // echoed back into the address bar (TC-AUTH-012).
  ...
  // Drop an unsafe redirect param from the address bar (history entry only).
  useEffect(() => {
    if (rawRedirect && redirect === '/discover') {
      const clean = new URLSearchParams(location.search);
      clean.delete('redirect');
    ...
    <div className="min-h-screen flex flex-col justify-between py-6 px-4 sm:px-6 lg:px-8 relative overflow-hidden bg-bg">
      ...
      <div className="w-full max-w-6xl mx-auto flex items-center justify-between z-20">
        ...
        <Link to="/discover" className="flex items-center gap-2.5 group">
          <div className="w-9 h-9 rounded-xl bg-white/10 dark:bg-white/5 border border-black/5 dark:border-white/10 shadow-sm flex items-center justify-center backdrop-blur-md group-hover:-translate-y-0.5 transition-transform">
        ...
        >
          ...
          >
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              <div>
                <label htmlFor="login-email" className="block text-xs font-bold uppercase tracking-wider text-text-secondary mb-1.5">
                  Email Address
                </label>
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label htmlFor="login-password" className="block text-xs font-bold uppercase tracking-wider text-text-secondary">
                    Password
                  </label>
                  <Link
                    to="/forgot-password"
              ...
              <div className="flex flex-col items-center pt-2 gap-2">
                <Turnstile
                  ...
                  onError={() => {
                    ...
                    setTurnstileError('Bot check failed to load. Please retry or check your connection.');
                  }}
                  onExpire={() => {
                    setTurnstileToken(null);
                    setTurnstileError('Bot check expired. Please verify again.');
                  }}
                  options={{ theme: 'dark' }}
                />
                {turnstileError && (
                  <div className="flex items-center gap-2 text-xs text-red-400">
                    ...
                    <button
                      ...
                      className="px-2.5 py-1 rounded-lg bg-bg-card border border-border text-text-secondary hover:text-text hover:border-accent/40 text-[11px] font-bold transition-colors touch-target"
                    >
                      Retry
                    </button>
                  </div>
