...
import { ArrowRight, Cpu, Wrench, ShieldCheck, Activity, Server, Zap, Database, Clock, Sparkles } from 'lucide-react';
...
import LazySection from '@/components/openlyst/LazySection';
...
import RisingStars from '@/components/openlyst/RisingStars';
import PersonalizedFeed from '@/components/openlyst/PersonalizedFeed';
...
export default function Home() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 rounded-lg relative">
      ...
      <AnimatePresence>
        {showStickyFilters && (
          <motion.div
            ...
            className="fixed top-20 left-1/2 -translate-x-1/2 z-[50] bg-bg-surface/80 backdrop-blur-xl border border-border rounded-full shadow-2xl p-2 flex items-center gap-1 w-[calc(100vw-2rem)] max-w-3xl overflow-x-auto no-scrollbar"
          >
            <span className="sticky left-0 shrink-0 flex items-center pl-1 pr-2 text-text-muted" aria-hidden="true">
            ...
            <span className="sticky right-0 shrink-0 flex items-center pl-3 pr-1 pointer-events-none" aria-hidden="true">
              <span className="w-6 h-6 rounded-full bg-bg-surface/90 backdrop-blur-md border border-border/60 flex items-center justify-center text-text-muted">
      ...
      <section ref={heroRef} className="pt-8 sm:pt-12 md:pt-16 pb-6 text-center max-w-5xl mx-auto px-4 overflow-hidden">
        <motion.div
          initial={{ opacity: 0, y: 16 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.4 }}>
          
          <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black tracking-[-0.04em] text-text leading-[1.05] mb-5 w-full flex flex-wrap items-center justify-center gap-x-2 sm:gap-x-3 overflow-hidden">
      ...
      {/* Below-fold sections render only when scrolled near (LazySection) to
          keep the initial DOM and paint light — the data still pre-fetches
          via the Home-level queries, so they appear instantly when reached. */}
      <LazySection>
        <RisingStars />
      </LazySection>
