...
import SponsoredBadge from '@/components/openlyst/SponsoredBadge';
import { useSponsorships, findPlacement } from '@/lib/sponsorships';
...
function AlternativeCard({ alt, idx, viewMode, isSelected, onToggleCompare, onSelect }) {
  ...
  const placement = findPlacement(sponsorshipsData?.placements, 'alternative', [
    alt?.free_tool_name,
    alt?.resolved_name,
    alt?.free_tool_repo,
  ]);
  ...
  return (
    <motion.div
      ...
      role="button"
      tabIndex={0}
      aria-label={`View details of ${alt?.resolved_name || 'this tool'}`}
      className="relative cursor-pointer outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2 focus-visible:ring-offset-bg rounded-xl"
    ...
    >
      ...
      >
        <div 
          ...
          style={{
            opacity: hovered ? 1 : 0,
            background: `radial-gradient(circle at ${spotlight.x}% ${spotlight.y}%, rgba(var(--accent-rgb, 99, 102, 241), 0.10) 0%, rgba(var(--accent-rgb, 99, 102, 241), 0.04) 35%, transparent 70%)`
          }}
        />
      </motion.div>
      ...
      >
        <button
          ...
          aria-label={`Compare ${alt.resolved_name || alt.free_tool_name || alt.repo?.name || 'this tool'}`}
        ...
        {viewMode === 'grid' ? (
              ...
              <div className="flex-1 min-w-0 pr-2">
                <div className="flex items-center gap-2">
                  <h3 className="text-sm font-bold text-text group-hover:text-accent transition-colors truncate">
                    {alt.resolved_name || alt.free_tool_name || alt.repo?.name || 'Open-source alternative'}
                  </h3>
                  {placement && (
                    <SponsoredBadge label={placement.badge_label} sponsorName={placement.sponsor_name} />
                  )}
                </div>
                <p className="text-xs font-medium text-text-muted mt-0.5 flex items-center gap-1.5">
                  ...
                  {(alt.verified_oss !== false || alt.source) && (
                    <span
                      title={alt.source ? `Source: ${alt.source}` : 'Verified open-source project'}
                      className="inline-flex items-center gap-0.5 text-[9px] uppercase font-bold tracking-wider px-1.5 py-0.5 rounded border border-emerald-500/30 bg-emerald-500/10 text-emerald-600 dark:text-emerald-300"
                    >
                      <Shield className="w-2.5 h-2.5" /> {alt.source ? alt.source.replace('_', ' ').slice(0, 14) : 'Verified'}
                    </span>
                  )}
                </p>
              </div>
          ...
          <>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <h3 className="text-sm font-bold text-text group-hover:text-accent transition-colors truncate">{alt.resolved_name || alt.free_tool_name || alt.repo?.name || 'Open-source alternative'}</h3>
                {placement && (
                  <SponsoredBadge label={placement.badge_label} sponsorName={placement.sponsor_name} />
                )}
                ...
                {(alt.verified_oss !== false || alt.source) && (
                  <span
                    ...
                    className="inline-flex items-center gap-0.5 text-[9px] uppercase font-bold tracking-wider px-1.5 py-0.5 rounded border border-emerald-500/30 bg-emerald-500/10 text-emerald-600 dark:text-emerald-300"
                  >
                    <Shield className="w-2.5 h-2.5" /> {alt.source ? alt.source.replace('_', ' ').slice(0, 12) : 'Verified'}
                  </span>
                )}
              </div>
...
export default function Alternatives() {
  ...
  // Modal/drawer focus management: trap Tab, focus first control, restore to opener on close
  useModalFocus({ active: !!selectedAlt, containerRef: detailModalRef, onClose: () => setSelectedAlt(null) });
  useModalFocus({ active: categoryDrawerOpen, containerRef: categoryDrawerRef, onClose: () => setCategoryDrawerOpen(false) });
  ...
  // Skip the initial mount run: firing on mount schedules a reset that wipes
  // accumulated results ~300ms after a cache-warm second visit has already
  // rendered them (the “second visit shows only Load More” bug).
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8 relative">
        ...
        <div className="flex flex-wrap items-center justify-between gap-2.5 sm:gap-3 mt-5 pt-4 sm:pt-5 border-t border-border/50">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button 
              ...
              aria-expanded={categoryDrawerOpen || sidebarOpen}
            >
      ...
      <div className="lg:hidden mb-5 -mx-4 px-4 sm:-mx-6 sm:px-6">
        <div className="flex items-center gap-2 overflow-x-auto no-scrollbar touch-scroll py-1">
          ...
          >
            <span>All Tools</span>
            <span className={`text-[10px] px-1.5 py-0.2 rounded-full ${activeCategory === 'All' ? 'bg-black/20 text-accent-fg' : 'bg-bg-subtle text-text-muted'}`}>
          ...
          {filteredData?.categories?.slice(0, visibleSidebarCategories).map((cat) => (
            ...
            >
              ...
              <span>{cat.name}</span>
              <span className={`text-[10px] px-1.5 py-0.2 rounded-full ${activeCategory === cat.name ? 'bg-black/20 text-accent-fg' : 'bg-bg-subtle text-text-muted'}`}>
                {cat.count}
              </span>
            </button>
          ))}
              ...
              <div className="w-[260px] sticky top-20">
                <div className="bg-bg-card border border-border rounded-xl overflow-hidden shadow-sm">
                  ...
                  <div className="max-h-[calc(100vh-220px)] overflow-y-auto custom-scrollbar p-1.5 space-y-0.5">
                    ...
                    >
                      <span>All Tools</span>
                      <span className={`text-xs px-1.5 py-0.5 rounded font-bold ${activeCategory === 'All' ? 'bg-black/20 text-accent-fg' : 'bg-bg-subtle text-text-muted'}`}>{filteredData?.stats?.total_tools || 0}</span>
                    </button>
                    {filteredData?.categories?.slice(0, visibleSidebarCategories).map((cat) => (
                      <button
                        key={cat.name}
                      ...
                      >
                        ...
                        <span className={`text-xs px-1.5 py-0.5 rounded font-bold flex-shrink-0 ${activeCategory === cat.name ? 'bg-black/20 text-accent-fg' : 'bg-bg-subtle text-text-muted'}`}>{cat.count}</span>
                      </button>
                    ))}
          ...
          >
            ...
            <div className="flex items-center gap-3 pl-4 border-l border-border">
              ...
              <button
                ...
                className="bg-text text-bg px-5 py-2 rounded-full text-sm font-bold hover:-translate-y-0.5 transition-transform"
              >
      ...
      <AnimatePresence>
        {selectedAlt && (
          <div id="alt-detail-modal" ref={detailModalRef} role="dialog" aria-modal="true" aria-labelledby="modal-title" className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
            ...
            >
               ...
               <div className="p-6 border-b border-border flex justify-between items-start bg-bg-subtle/50">
                  ...
                  <button onClick={() => setSelectedAlt(null)} className="p-2 hover:bg-bg rounded-lg text-text-muted hover:text-text transition-colors" aria-label="Close details">
                    ✕
                  </button>
               </div>
                      ...
                      <div className="bg-bg-subtle rounded-2xl p-4 sm:p-5 border border-border">
                        ...
                        >
                          <div className="w-10 h-10 rounded-lg bg-red-500/10 flex items-center justify-center flex-shrink-0">
                            <PlayCircle className="w-5 h-5 text-red-500 group-hover:-translate-y-0.5 transition-transform" />
                          </div>
                          <div>
      ...
      <AnimatePresence>
        {categoryDrawerOpen && (
          <div id="alt-category-drawer" ref={categoryDrawerRef} role="dialog" aria-modal="true" aria-label="Select category" className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 backdrop-blur-sm lg:hidden">
            <motion.div 
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-1 custom-scrollbar touch-scroll">
                ...
                >
                  <span>All Tools</span>
                  <span className={`text-xs px-2 py-0.5 rounded-full font-bold ${activeCategory === 'All' ? 'bg-black/20 text-accent-fg' : 'bg-bg-subtle text-text-muted'}`}>
                ...
                {data?.categories?.slice(0, visibleSidebarCategories).map((cat) => (
                  ...
                  >
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full font-bold flex-shrink-0 ${activeCategory === cat.name ? 'bg-black/20 text-accent-fg' : 'bg-bg-subtle text-text-muted'}`}>
                      {cat.count}
                    </span>
                  </button>
                ))}
