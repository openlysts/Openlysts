...
import { getBookmarks, removeBookmark, persistBookmarkToggle, fetchAccountBookmarks, mergeAccountBookmarkIds } from '@/lib/bookmarks';
...
export default function Bookmarks() {
  ...
  // When signed in, pull the account list and merge it into the local one so
  // bookmarks made on other devices appear here too.
  useEffect(() => {
    if (!isAuthenticated) return;
    let alive = true;
    fetchAccountBookmarks().then((rows) => {
      if (!alive || !rows) return;
      mergeAccountBookmarkIds(rows.map((r) => r.repository_id));
    });
    return () => { alive = false; };
  }, [isAuthenticated]);
  ...
  const handleRemove = (id) => {
    removeBookmark(id);
    persistBookmarkToggle(id, false, isAuthenticated);
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
      ...
      <p className="text-text-secondary text-sm mb-8">
        {bookmarkIds.length} saved {bookmarkIds.length === 1 ? 'repository' : 'repositories'}
        {isAuthenticated ? ' · synced to your account' : ' · stored locally in your browser'}
      </p>
