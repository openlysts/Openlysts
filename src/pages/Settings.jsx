...
import {
  Settings as SettingsIcon, Github, Eye, EyeOff, Trash2,
  Check, BookmarkX, RotateCcw, SlidersHorizontal, KeyRound,
  Shield, Smartphone, Fingerprint, Plus, X, Download, UserMinus,
  Accessibility as AccessibilityIcon, Type, Wind, Contrast, Focus
} from 'lucide-react';
...
export default function Settings() {
  ...
  const [githubTokens, setGithubTokens] = useState(() => {
    const existing = settings.githubToken || '';
    return existing ? existing.split(',').map(t => t.trim()).filter(Boolean) : [];
  });
  ...
  const handleAddToken = async () => {
    ...
    if (!token || githubTokens.includes(token)) return;
    const newTokens = [...githubTokens, token];
    setGithubTokens(newTokens);
    setTokenInput('');
    const updated = saveSettings({ githubToken: newTokens.join(',') });
    setSettings(updated);
    try {
      await updateConfig(newTokens.join(','));
      toast({ title: 'Token added', description: `Pool now has ${newTokens.length} token(s).` });
    } catch (e) {
      toast({ title: 'Token saved locally', description: 'Stored in your browser.' });
    }
  };

  const handleRemoveToken = async (index) => {
    ...
    try {
      await updateConfig(newTokens.join(','));
      toast({ title: 'Token removed', description: `Pool now has ${newTokens.length} token(s).` });
    } catch (e) {}
  };
  ...
  const handleWithdrawConsent = async () => {
    ...
    try {
      ...
      if (res.ok) {
        // Hard-erase local mirrors of account data (bookmarks + compare tray)
        clearBookmarks();
        localStorage.removeItem('openlyst_compare');
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-10">
      ...
      <div className="space-y-6">
        ...
        >
          ...
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <label className="text-sm font-bold text-text">GitHub Access Tokens</label>
              {githubTokens.length > 0 ? (
                <span className="ml-auto inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider text-oss bg-oss-soft px-2 py-0.5 rounded-full border border-oss/20">
                  <Check className="w-3 h-3" /> {githubTokens.length} token{githubTokens.length > 1 ? 's' : ''}
                </span>
              ) : (
            ...
            {githubTokens.length > 0 && (
              <div className="space-y-2 mb-3">
                {githubTokens.map((t, idx) => (
                  <div key={idx} className="flex items-center gap-2 bg-bg-subtle/60 rounded-xl px-3 py-2 border border-border/50">
                    <KeyRound className="w-3.5 h-3.5 text-text-muted flex-shrink-0" />
                    <span className="text-xs font-mono text-text-secondary flex-1 truncate">
                      {'•'.repeat(8)}...{t.slice(-6)}
                    </span>
                    <span className="text-[10px] font-bold text-text-muted">Token {idx + 1}</span>
                    <button onClick={() => handleRemoveToken(idx)} className="text-text-muted hover:text-nonoss transition-colors" aria-label="Remove token">
            ...
            {/* Add new token */}
            <form className="flex gap-2" onSubmit={(e) => { e.preventDefault(); handleAddToken(); }}>
            ...
            <div className="flex items-center justify-between mt-3">
              <p className="text-xs text-text-muted/80 font-medium">
                Add multiple tokens for automatic rotation. Each token gets 5,000 req/hr. Create at{' '}
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between p-3 rounded-2xl hover:bg-bg-subtle transition-colors group">
              ...
              <button
                ...
                className={`relative w-11 h-6 rounded-full transition-colors duration-300 shadow-inner border border-black/10 dark:border-white/10 ${settings.autoExpandVideos ? 'bg-accent' : 'bg-border-strong'}`}
              >
        ...
        <motion.section 
          initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.18, ease: EASE_OUT }}
          className="bg-bg-card/90 backdrop-blur-xl p-6 rounded-3xl border border-border/80 shadow-xl"
        >
          <div className="flex items-center gap-2 mb-1">
            <AccessibilityIcon className="w-5 h-5 text-accent" />
            <h2 className="font-bold text-lg text-text">Accessibility</h2>
          </div>
          <p className="text-xs font-medium text-text-muted mb-5">
            Make Openlysts work for you — these preferences apply instantly across every page and respect WCAG AA.
          </p>

          <div className="space-y-4">
            {/* Text size */}
            <div className="p-3 rounded-2xl hover:bg-bg-subtle transition-colors">
              <div className="flex items-center gap-2 mb-2">
                <Type className="w-4 h-4 text-accent" />
                <p className="text-sm font-bold text-text">Text size</p>
              </div>
              <div className="flex gap-2" role="radiogroup" aria-label="Text size">
                {[['normal', 'Normal'], ['large', 'Large'], ['xlarge', 'Extra large']].map(([val, label]) => (
                  <button
                    key={val}
                    role="radio"
                    aria-checked={(settings.fontSize || 'normal') === val}
                    onClick={() => handlePrefChange('fontSize', val)}
                    className={`flex-1 px-3 py-2 rounded-xl border text-xs font-bold transition-all shadow-sm ${
                      (settings.fontSize || 'normal') === val
                        ? 'border-accent/60 bg-accent/10 text-accent'
                        : 'border-border/80 bg-bg text-text-secondary hover:bg-bg-subtle'
                    }`}
                  >
                    {label}
                  </button>
            ...
            <div className="p-3 rounded-2xl hover:bg-bg-subtle transition-colors">
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-sm font-bold text-text">Motion &amp; animations</p>
              </div>
              <div className="flex gap-2" role="radiogroup" aria-label="Motion and animations">
                {[['system', 'Follow device'], ['reduced', 'Reduce motion'], ['full', 'Full motion']].map(([val, label]) => (
                  <button
                    key={val}
                    role="radio"
                    aria-checked={(settings.reducedMotion || 'system') === val}
                    onClick={() => handlePrefChange('reducedMotion', val)}
                    className={`flex-1 px-3 py-2 rounded-xl border text-xs font-bold transition-all shadow-sm ${
                      (settings.reducedMotion || 'system') === val
                        ? 'border-accent/60 bg-accent/10 text-accent'
                        : 'border-border/80 bg-bg text-text-secondary hover:bg-bg-subtle'
                    }`}
                  >
            ...
            <div className="flex items-center justify-between p-3 rounded-2xl hover:bg-bg-subtle transition-colors group">
              <div className="flex items-start gap-2">
                <Contrast className="w-4 h-4 text-accent mt-0.5" />
                <div>
                  <p className="text-sm font-bold text-text">High contrast</p>
                  <p className="text-xs font-medium text-text-muted mt-0.5">Darken or brighten secondary text and borders.</p>
                </div>
              </div>
              <button
                role="switch"
                ...
                onClick={() => handlePrefChange('highContrast', !settings.highContrast)}
                className={`relative w-11 h-6 rounded-full transition-colors duration-300 shadow-inner border border-black/10 dark:border-white/10 shrink-0 ${settings.highContrast ? 'bg-accent' : 'bg-[hsl(217,25%,47%)]'}`}
              >
                <span className={`absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-white shadow-sm transition-transform duration-300 ${settings.highContrast ? 'translate-x-5' : ''}`} />
              </button>
            </div>
            ...
            <div className="flex items-center justify-between p-3 rounded-2xl hover:bg-bg-subtle transition-colors group">
              <div className="flex items-start gap-2">
                <Focus className="w-4 h-4 text-accent mt-0.5" />
                <div>
                  <p className="text-sm font-bold text-text">Strong focus indicators</p>
                  <p className="text-xs font-medium text-text-muted mt-0.5">Highly visible outlines when navigating with the keyboard.</p>
                </div>
              </div>
              <button
                role="switch"
                aria-checked={!!settings.strongFocus}
                aria-label="Strong focus indicators"
                onClick={() => handlePrefChange('strongFocus', !settings.strongFocus)}
                className={`relative w-11 h-6 rounded-full transition-colors duration-300 shadow-inner border border-black/10 dark:border-white/10 shrink-0 ${settings.strongFocus ? 'bg-accent' : 'bg-[hsl(217,25%,47%)]'}`}
              >
                <span className={`absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-white shadow-sm transition-transform duration-300 ${settings.strongFocus ? 'translate-x-5' : ''}`} />
              </button>
            </div>
          </div>
        </motion.section>
