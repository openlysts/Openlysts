...
// three.js (via Scanner) only loads when the deferred background actually starts
const LazyScanner = lazy(() => import('../components/reactbits/Scanner'));
...
export default function Welcome() {
  ...
  // The WebGL scanner background is a full-screen continuous shader — start it
  // only after the hero text has painted (past the LCP window) and never for
  // prefers-reduced-motion users, so it cannot block first content.
  useEffect(() => {
    if (typeof window === 'undefined') return;
    if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
    const timer = setTimeout(() => setScannerOn(true), 3000);
    return () => clearTimeout(timer);
  }, []);
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden bg-black selection:bg-accent/30 flex items-center justify-center">
      ...
      {/* Scanner Animated Background (deferred until after LCP; see effect above) */}
      ...
      <div className="absolute top-6 right-6 md:top-8 md:right-8 z-50">
        ...
        >
          <span className="relative z-10">Why Openlysts</span>
          <div className="absolute inset-0 rounded-full border border-white/10 bg-white/5 backdrop-blur-md transition-all duration-300 group-hover:bg-white/10 group-hover:border-white/20 group-hover:-translate-y-0.5" />
        </button>
      </div>

      {/* Why Openlysts Modal Overlay */}
      <AnimatePresence>
        {isAboutOpen && (
