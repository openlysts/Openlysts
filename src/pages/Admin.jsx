...
import { 
  ...
  Radio, Clock, Cpu, HardDrive, Zap, Loader2, Compass, Play, X, XCircle, CheckSquare, Square, Gauge,
  Coins, Megaphone, CalendarClock, Pencil
} from 'lucide-react';
...
// Compact month grid of sponsored placements (live amber / scheduled blue).
function PlacementsCalendar({ placements }) {
  ...
  const active = (p) => {
    ...
    const now = Date.now();
    if (p.active === 0 || p.active === false || p.active === '0') return false;
    if (s && now < s.getTime()) return false;
    if (e && now > e.getTime()) return false;
    return true;
  };
    ...
    <div className="card p-5">
      <div className="flex items-center justify-between mb-3">
        <h3 className="font-bold text-text text-sm flex items-center gap-2">
          <CalendarClock className="w-4 h-4 text-amber-400" /> Placements calendar
        </h3>
        <div className="flex items-center gap-3 text-[10px] font-semibold text-text-secondary">
          ...
          <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-sky-400 inline-block" /> Scheduled</span>
        </div>
      </div>
      <p className="text-[11px] text-text-muted -mt-1 mb-3">{monthLabel} — hover a day for its placements</p>
      <div className="grid grid-cols-7 gap-1 text-center">
        {dayNames.map((dn) => (
          <div key={dn} className="text-[10px] font-bold text-text-muted py-1">{dn}</div>
        ))}
        {cells.map((day, i) => {
          if (day === null) return <div key={`x${i}`} />;
          const date = new Date(year, month, day);
          const dayPlac = placements.filter((p) => {
            const s = p.starts_at ? new Date(p.starts_at) : null;
            const e = p.ends_at ? new Date(p.ends_at) : null;
            if (s && date < new Date(s.getFullYear(), s.getMonth(), s.getDate())) return false;
            if (e && date > new Date(e.getFullYear(), e.getMonth(), e.getDate())) return false;
            return true;
          });
          const isToday = day === today.getDate();
          const hasLive = dayPlac.some(active);
          const hasSched = dayPlac.some((p) => !active(p) && p.active !== 0 && p.active !== false && p.active !== '0');
          return (
            <div
              key={day}
              title={dayPlac.length ? dayPlac.map((p) => `${p.target_name} (${p.badge_label})`).join(' · ') : ''}
              className={`relative aspect-square rounded-lg border text-[11px] font-semibold flex items-center justify-center transition-colors ${isToday ? 'border-accent text-accent' : 'border-border/40 text-text-secondary'}`}
            >
              {day}
              {(hasLive || hasSched) && (
                <span className="absolute bottom-0.5 left-1/2 -translate-x-1/2 flex gap-0.5">
                  {hasLive && <span className="w-1 h-1 rounded-full bg-amber-400" />}
                  {hasSched && <span className="w-1 h-1 rounded-full bg-sky-400" />}
                </span>
              )}
            </div>
          );
      ...
      <p className="text-[11px] text-text-muted mt-3">{monthLabel} view · open-ended placements (no end date) show through the whole month.</p>
    </div>
  );
}

// Admin-tunable Neon Transfer Guard thresholds (persisted via SystemConfig;
// the guard reads them with env fallback — see server/services/transferGuard.js).
function TransferThresholds({ warnPct = 70, criticalPct = 85, onSaved }) {
  const [warn, setWarn] = useState(Number(warnPct) || 70);
  const [critical, setCritical] = useState(Number(criticalPct) || 85);
  const [saving, setSaving] = useState(false);
  const { toast } = useToast();

  async function save() {
    if (critical <= warn) {
      toast({ title: 'Invalid thresholds', description: 'Critical must be higher than warn.', variant: 'destructive' });
      return;
    }
    setSaving(true);
    try {
      for (const [key, value] of [['transfer_guard_warn_pct', String(warn)], ['transfer_guard_critical_pct', String(critical)]]) {
        ...
        if (!res.ok || data.error) throw new Error(data.message || 'Save failed');
      }
      toast({ title: 'Thresholds saved', description: `Guard switches at ${warn}% (warn) / ${critical}% (critical).` });
      onSaved?.();
    } catch (err) {
  ...
  return (
    <div className="card p-5">
      <h3 className="font-bold text-text text-sm mb-1 flex items-center gap-2">
        <Gauge className="w-4 h-4 text-amber-400" /> Transfer Guard thresholds
      </h3>
      <p className="text-xs text-text-secondary mb-4">
        The guard switches the read path to RAM/edge-only when Neon network transfer crosses the critical %. No code — settings apply within a minute.
      </p>
      <div className="space-y-4">
        <div>
          <div className="flex justify-between text-xs mb-1.5">
            <span className="text-text-secondary font-medium">Warn at <span className="font-mono text-amber-400">{warn}%</span></span>
            <span className="text-text-muted text-[11px]">budget panel warns</span>
          </div>
          <input type="range" min="50" max="95" value={warn} onChange={(e) => setWarn(Number(e.target.value))} className="w-full accent-amber-400" aria-label="Warn threshold percent" />
        </div>
        <div>
          <div className="flex justify-between text-xs mb-1.5">
            <span className="text-text-secondary font-medium">Critical at <span className="font-mono text-trending">{critical}%</span></span>
            <span className="text-text-muted text-[11px]">RAM/edge-only below the Neon cap</span>
          </div>
          <input type="range" min="60" max="99" value={critical} onChange={(e) => setCritical(Number(e.target.value))} className="w-full accent-trending" aria-label="Critical threshold percent" />
        </div>
        <button
          type="button"
          onClick={save}
          disabled={saving}
          className="px-4 py-2 rounded-lg bg-accent text-accent-fg text-xs font-bold hover:opacity-90 disabled:opacity-50 transition-all flex items-center gap-1.5 touch-target"
        >
...
export default function Admin() {
  ...
  // ─── Budget Query ───────────────────────────────────────────────
  const { data: budget, refetch: refetchBudget, isLoading: budgetLoading, isFetching: budgetFetching } = useQuery({
    ...
    staleTime: 0, // Always consider stale so manual refresh works
    cacheTime: 0, // Don't cache so fresh data is always fetched
  });
  ...
  const { data: sponsorshipsData, refetch: refetchSponsorships } = useQuery({
    queryKey: ['admin-sponsorships'],
    queryFn: async () => {
      const res = await fetch('/api/admin/sponsorships', { credentials: 'include' });
      if (!res.ok) throw new Error('Failed to load sponsorships');
      return res.json();
    },
  });
  const placements = sponsorshipsData?.placements || [];
  ...
  const EMPTY_PLACEMENT = { placement_type: 'repository', target_name: '', badge_label: 'Sponsored', sponsor_name: '', sponsor_url: '', starts_at: '', ends_at: '', active: 1, sort_order: 0 };
  ...
  const [donationForm, setDonationForm] = useState({ donation_enabled: false, donation_label: 'Support Openlysts', donation_url: '' });
  const [donationSaving, setDonationSaving] = useState(false);

  // ─── API Tokens (Admin → API Tokens) ──────────────────────────────
  const { data: apiTokensData, refetch: refetchApiTokens } = useQuery({
  ...
  async function handleCreateToken(e) {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok || data.error) throw new Error(data.message || 'Failed to create token');
      setRevealedToken(data.token.token);
      setNewTokenName('');
      toast({ title: 'Token created', description: 'Copy it now — it is shown only once.' });
  ...
  async function handleRevokeToken(id) {
    if (!window.confirm('Revoke this API token? Existing callers will be rejected immediately.')) return;
    try {
      const res = await fetch(`/api/admin/api-tokens/${id}`, { method: 'DELETE', credentials: 'include' });
      const data = await res.json();
      if (!res.ok || data.error) throw new Error(data.message || 'Revoke failed');
  ...
  async function handleCopyToken() {
    ...
    } catch (e) { /* clipboard unavailable (non-secure context) */ }
  }
  ...
  const sponsorSuggestions = useMemo(() => {
    ...
    for (const a of alternatives) {
      if (a.free_tool_name) altNames.push(a.free_tool_name);
      if (a.free_tool_repo && a.free_tool_repo.includes('/') && !a.free_tool_repo.startsWith('http')) altNames.push(a.free_tool_repo);
    }
    return Array.from(new Set([...repoNames, ...altNames])).sort().slice(0, 500);
  }, [repos, alternatives]);

  // Keep the donation form in sync with the settings query.
  React.useEffect(() => {
    setDonationForm({
      donation_enabled: appSettings.donation_enabled === '1' || appSettings.donation_enabled === 'true',
      donation_label: appSettings.donation_label || 'Support Openlysts',
      donation_url: appSettings.donation_url || '',
    });
  }, [settingsData]);
  ...
  async function handleSavePlacement(e) {
    ...
    try {
      ...
      const url = editingPlacementId ? `/api/admin/sponsorships/${editingPlacementId}` : '/api/admin/sponsorships';
      const res = await fetch(url, {
        ...
        body: JSON.stringify({
          ...placementForm,
          starts_at: toUtcIso(placementForm.starts_at),
          ends_at: toUtcIso(placementForm.ends_at),
        }),
      });
      const data = await res.json();
      if (!res.ok || data.error) throw new Error(data.message || 'Failed to save placement');
      toast({ title: editingPlacementId ? 'Placement updated' : 'Placement created', description: `Badge will show on “${placementForm.target_name}”` });
  ...
  // datetime-local inputs carry the admin's LOCAL wall time with no zone;
  // the public filter compares UTC ISO text, so naive values silently excluded
  // live placements (stored "17:46" vs UTC "12:34…Z"). Convert to UTC ISO on
  // the way out and back to local wall time on the way in for the input.
  const toUtcIso = (v) => {
    if (!v) return '';
  ...
  function handleEditPlacement(p) {
    setEditingPlacementId(p.id);
    setPlacementForm({
      placement_type: p.placement_type || 'repository',
      target_name: p.target_name || '',
      badge_label: p.badge_label || 'Sponsored',
  ...
  async function handleDeletePlacement(p) {
    if (!window.confirm(`Delete the ${p.badge_label || 'Sponsored'} placement for “${p.target_name}”?`)) return;
    try {
      const res = await fetch(`/api/admin/sponsorships/${p.id}`, { method: 'DELETE', credentials: 'include' });
      const data = await res.json();
      if (!res.ok || data.error) throw new Error(data.message || 'Failed to delete');
      toast({ title: 'Placement deleted', description: `“${p.target_name}” is no longer sponsored.` });
      if (editingPlacementId === p.id) { setEditingPlacementId(null); setPlacementForm(EMPTY_PLACEMENT); }
      queryClient.invalidateQueries({ queryKey: ['admin-sponsorships'] });
    } catch (err) {
  ...
  async function handleSaveDonation(e) {
    ...
    try {
      const keys = [
        ['donation_enabled', donationForm.donation_enabled ? '1' : '0'],
        ['donation_label', donationForm.donation_label],
        ['donation_url', donationForm.donation_url],
      ];
      for (const [key, value] of keys) {
        ...
        const data = await res.json();
        if (!res.ok || data.error) throw new Error(data.message || `Failed to save ${key}`);
      }
      toast({ title: 'Donation settings saved', description: donationForm.donation_enabled ? 'The Support button is now visible in the footer.' : 'The Support button is hidden.' });
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-1.5 bg-bg-card p-1.5 rounded-2xl border border-border shadow-sm">
        {[
          ...
          { id: 'health', label: 'System Health', icon: Zap, badge: null },
          { id: 'config', label: 'System Config', icon: HardDrive, badge: config.length ? 'Active' : null },
          { id: 'budget', label: 'Free-Tier Budget', icon: Gauge, badge: null },
          { id: 'monetization', label: 'Monetization', icon: Coins, badge: placements.length ? `${placements.length}` : null },
          { id: 'apiTokens', label: 'API Tokens', icon: KeyRound, badge: apiTokens.length ? `${apiTokens.length}` : null },
        ].map((tab) => {
          const Icon = tab.icon;
      ...
      {/* ─── TAB: FREE-TIER BUDGET ─── */}
      {activeTab === 'budget' && (
        <div className="space-y-6">
          {budget?.transferGuard?.mode === 'critical' && (
            <div className="rounded-xl border border-trending/40 bg-trending/10 p-4 text-xs text-text leading-relaxed">
              <span className="font-bold text-trending">🚨 Neon transfer critical ({budget.transferGuard.criticalPct}%+ of the 5 GB cap):</span>{' '}
              reads are now RAM/edge-only — Neon syncs, enrichment and live table counts are paused until usage drops. The catalogue stays fully searchable; freshness resumes automatically next billing period.
            </div>
          )}
          {budget?.transferGuard?.mode === 'warn' && (
            <div className="rounded-xl border border-amber-400/40 bg-amber-400/10 p-4 text-xs text-text leading-relaxed">
              <span className="font-bold text-amber-400">⚠️ Neon transfer above {budget.transferGuard.warnPct}%:</span>{' '}
              still serving full reads. Crossing {budget.transferGuard.criticalPct}% switches the read path to RAM/edge-only automatically.
            </div>
          )}
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div className="card p-5">
              <h3 className="font-bold text-text text-sm mb-1 flex items-center gap-2">
                <Database className="w-4 h-4 text-accent" /> Neon Storage
              </h3>
              <p className="text-xs text-text-secondary mb-4">Live, refreshed from PostgreSQL</p>
              {(() => {
                const dbBytes = budget?.neon?.dbBytes || 0;
                const dbCap = budget?.neon?.dbCapBytes || 1;
                const ltBytes = budget?.neon?.longTailBytes || 0;
                const ltCap = budget?.neon?.longTailCapBytes || 1;
                  ...
                  <div className="space-y-4">
                    <div>
                      <div className="flex justify-between text-xs mb-1.5">
                        ...
                        <span className="font-mono text-text">{fmt(dbBytes)} <span className="text-text-muted">/ {fmt(dbCap)}</span></span>
                      </div>
                      <div className="w-full h-2 rounded-full bg-bg-subtle overflow-hidden">
                        <div className={`h-full transition-all duration-500 rounded-full ${dbBytes / dbCap > 0.8 ? 'bg-trending' : 'bg-accent'}`} style={{ width: `${Math.min(100, (dbBytes / dbCap) * 100)}%` }} />
                      </div>
                    </div>
                    <div>
                      <div className="flex justify-between text-xs mb-1.5">
                        <span className="text-text-secondary font-medium">Long-tail tier</span>
                        <span className="font-mono text-text">{fmt(ltBytes)} <span className="text-text-muted">/ {fmt(ltCap)}</span></span>
                      </div>
                      <div className="w-full h-2 rounded-full bg-bg-subtle overflow-hidden">
                        <div className={`h-full transition-all duration-500 rounded-full ${ltBytes / ltCap > 0.8 ? 'bg-trending' : 'bg-accent'}`} style={{ width: `${Math.min(100, (ltBytes / ltCap) * 100)}%` }} />
                      </div>
                    </div>
                    {budget?.neon?.transfer ? (
                      <div>
                        <div className="flex justify-between text-xs mb-1.5">
                          <span className="text-text-secondary font-medium">Network transfer <span className="text-[9px] uppercase tracking-wide">(5 GB/mo free cap)</span></span>
                          <span className={`font-mono font-bold ${budget.neon.transfer.percent > 80 ? 'text-trending' : 'text-text'}`}>
                            {budget.neon.transfer.percent >= 1 ? `${budget.neon.transfer.percent.toFixed(1)}%` : '<1%'}
                          </span>
                        </div>
                        <div className="w-full h-2 rounded-full bg-bg-subtle overflow-hidden">
                          <div
                            className={`h-full transition-all duration-500 rounded-full ${budget.neon.transfer.percent > 80 ? 'bg-trending' : budget.neon.transfer.percent > 60 ? 'bg-amber-400' : 'bg-emerald-500'}`}
                            style={{ width: `${Math.min(100, budget.neon.transfer.percent)}%` }}
                          />
                        </div>
                        <p className="text-[11px] text-text-muted mt-1">
                          {(budget.neon.transfer.usedBytes / 1073741824).toFixed(2)} GB used · overage suspends compute, never deletes data
                        </p>
                      </div>
                    ) : (
                      <p className="text-[11px] text-text-muted">
                        Network transfer: unknown — set <span className="font-mono">NEON_API_KEY</span> + <span className="font-mono">NEON_PROJECT_ID</span> to monitor the 5 GB/mo cap.
                      </p>
                    )}
                    <p className="text-[11px] text-text-muted">
                      Hot cap {(budget?.neon?.hotCapRows || 0).toLocaleString()} rows · demote at {(budget?.neon?.demoteTriggerRows || 0).toLocaleString()} · measured {budget?.neon?.measuredAt ? new Date(budget.neon.measuredAt).toLocaleTimeString() : '—'}
                    </p>
                  </div>
                );
              })()}
            ...
            <div className="card p-5">
              <h3 className="font-bold text-text text-sm mb-1 flex items-center gap-2">
                <RefreshCw className="w-4 h-4 text-amber-400" /> GitHub Actions
              </h3>
              <p className="text-xs text-text-secondary mb-4">Live from the GitHub API</p>
              {budget?.actions?.status === 'ok' ? (
                <div className="space-y-3">
                  <span className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-bold ${budget.actions.repoPublic ? 'bg-oss-soft text-oss' : 'bg-amber-500/10 text-amber-400'}`}>
                    {budget.actions.repoPublic ? 'Public repo — free, unlimited' : 'Private repo'}
                  </span>
                  <div>
                    <div className="flex justify-between text-xs mb-1.5">
                      <span className="text-text-secondary font-medium">Minutes · current billing cycle</span>
                      <span className="font-mono text-text">{budget.actions.usedMinutes} min{budget.actions.limitMinutes ? ` / ${budget.actions.limitMinutes.toLocaleString()} / mo` : ''}</span>
                    </div>
                    {budget.actions.limitMinutes && (
                      <div className="w-full h-2 rounded-full bg-bg-subtle overflow-hidden">
                        <div className="h-full bg-amber-400 transition-all duration-500 rounded-full" style={{ width: `${Math.min(100, (budget.actions.usedMinutes / budget.actions.limitMinutes) * 100)}%` }} />
                      </div>
                    )}
                  </div>
                  <p className="text-[11px] text-text-muted">{budget.actions.limitLabel}</p>
                </div>
              ) : ['snapshot', 'billing-stale', 'snapshot-sum'].includes(budget?.actions?.status) ? (
                <div className="space-y-3">
                  <span className="inline-flex px-2 py-0.5 rounded-full text-[10px] font-bold bg-sky-500/10 text-sky-400">
                    {budget.actions.status === 'billing-stale' ? 'Neon snapshot — billing stale' : 'Neon snapshot — GitHub API limited'}
                  </span>
                  <div>
                    <div className="flex justify-between text-xs mb-1.5">
                      <span className="text-text-secondary font-medium">Monthly minutes · true total</span>
                      <span className="font-mono text-text">{budget.actions.usedMinutes ?? budget.actions.monthlyMinutes ?? '—'} min</span>
                    </div>
                  </div>
                  <p className="text-[11px] text-text-muted">
                    {budget.actions.snapshotDays != null ? `${budget.actions.snapshotDays} daily snapshot(s) · ` : ''}
                    {budget.actions.snapshotDate ? `last ${new Date(budget.actions.snapshotDate).toLocaleDateString()}` : 'no snapshots yet'}
                    {budget?.actions?.reason ? ` — ${budget.actions.reason}` : ''}
                  </p>
                </div>
              ) : (
                <p className="text-xs text-text-muted">Unavailable: {budget?.actions?.reason || 'no token'}</p>
              )}
            </div>
            ...
            <div className="card p-5">
              <h3 className="font-bold text-text text-sm mb-1 flex items-center gap-2">
                <Zap className="w-4 h-4 text-blue-400" /> Vercel Bandwidth (estimate)
              </h3>
              <p className="text-xs text-text-secondary mb-4">Measured in this process</p>
              {(() => {
                const used = budget?.vercel?.apiBytesSinceBoot || 0;
                const cap = budget?.vercel?.limitBytes || 1;
                return (
                  <div>
                    <div className="flex justify-between text-xs mb-1.5">
                      <span className="text-text-secondary font-medium">API bytes since boot</span>
                      <span className="font-mono text-text">{(used / 1048576).toFixed(1)} MB <span className="text-text-muted">/ 100 GB</span></span>
                    </div>
                    <div className="w-full h-2 rounded-full bg-bg-subtle overflow-hidden">
                      <div className="h-full bg-blue-400 transition-all duration-500 rounded-full" style={{ width: `${Math.min(100, (used / cap) * 100)}%` }} />
                    </div>
                    <p className="text-[11px] text-text-muted mt-3">{budget?.vercel?.note || ''}</p>
                  </div>
                );
              })()}
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <TransferThresholds
              warnPct={budget?.transferGuard?.warnPct}
              criticalPct={budget?.transferGuard?.criticalPct}
              onSaved={refetchBudget}
            />
          </div>
          <button
            onClick={() => refetchBudget()}
            disabled={budgetFetching}
            className="px-3 py-1.5 rounded-lg bg-bg-card border border-border text-xs font-semibold text-text-secondary hover:text-text hover:border-accent/40 transition-all flex items-center gap-1.5 disabled:opacity-50"
          >
            <RefreshCw className={`w-3.5 h-3.5 ${budgetFetching ? 'animate-spin' : ''}`} /> {budgetFetching ? 'Refreshing...' : 'Refresh'}
          </button>
        </div>
      )}
      ...
      <AnimatePresence>
        ...
        {activeTab === 'apiTokens' && (
          <div className="space-y-6">
            <div className="card p-5 sm:p-6">
              <h3 className="font-bold text-text text-sm flex items-center gap-2">
                <KeyRound className="w-4 h-4 text-accent" /> API Tokens
              </h3>
              <p className="text-xs text-text-secondary mt-1 mb-4">
                Bearer tokens for external callers of the read endpoints (search repositories, alternatives, stats). A valid token gets rate-limit relief for reads — admin-only functions always need an admin session. The secret is shown only once at creation.
              </p>

              {/* Create */}
              <form onSubmit={handleCreateToken} className="flex flex-col sm:flex-row gap-2 mb-5">
                <input
                  className="flex-1 bg-bg border border-border rounded p-2 text-sm text-text"
                  placeholder="Token name (e.g. My blog embed)"
                  value={newTokenName}
              ...
              {/* Revealed once */}
              {revealedToken && (
                <div className="mb-5 p-4 rounded-xl border border-emerald-500/40 bg-emerald-500/10 text-xs">
                  <p className="font-bold text-emerald-500 mb-1.5">Copy your token now — it will not be shown again.</p>
                  <div className="flex items-center gap-2">
                    <code className="flex-1 break-all bg-bg border border-border rounded-lg p-2 text-text font-mono text-[11px]">{revealedToken}</code>
                    <button
                      type="button"
                      onClick={handleCopyToken}
                      className="px-3 py-2 rounded-lg bg-bg-card border border-border text-xs font-semibold text-text-secondary hover:text-text transition-all flex items-center gap-1.5"
                    >
                      {copied ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />} {copied ? 'Copied' : 'Copy'}
                    </button>
                  </div>
                  <p className="text-[11px] text-text-muted mt-2">Use as: <code className="font-mono">Authorization: Bearer {revealedToken.slice(0, 8)}…</code> on <code className="font-mono">/api/functions/queryRepositories</code> etc.</p>
                </div>
              )}

              {/* List */}
              {apiTokens.length === 0 ? (
                <p className="text-xs text-text-muted bg-bg-subtle border border-dashed border-border rounded-xl p-4">No API tokens yet — create one above.</p>
              ) : (
                  ...
                  <table className="w-full text-left text-xs min-w-[560px]">
                    <thead>
                      <tr className="text-text-muted border-b border-border">
                        ...
                        <th className="py-2 pr-3 font-semibold">Key prefix</th>
                    ...
                    <tbody>
                      {apiTokens.map((t) => (
                        <tr key={t.id} className="border-b border-border/40">
                          <td className="py-2.5 pr-3 font-semibold text-text">{t.name}</td>
                          <td className="py-2.5 pr-3"><code className="font-mono text-[11px] bg-bg-subtle px-1.5 py-0.5 rounded border border-border">{t.prefix}…</code></td>
                          <td className="py-2.5 pr-3 text-text-secondary">{new Date(t.created_at).toLocaleDateString()}</td>
                          <td className="py-2.5 pr-3 text-text-secondary">{t.last_used_at ? new Date(t.last_used_at).toLocaleString() : 'never'}</td>
                          <td className="py-2.5 pr-3">
                            {t.revoked_at ? (
                              <span className="inline-flex px-2 py-0.5 rounded-full text-[10px] font-bold bg-trending/10 text-trending">Revoked</span>
                            ) : (
                              <span className="inline-flex px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-500/10 text-emerald-500">Active</span>
                            )}
                          </td>
                          <td className="py-2.5">
                            {!t.revoked_at && (
                              <button
                                ...
                                className="text-[11px] font-semibold text-trending hover:underline flex items-center gap-1"
                              >
        ...
        {activeTab === 'monetization' && (
          <div className="space-y-6">
            ...
            <div className="card p-5 sm:p-6">
              <h3 className="font-bold text-text text-sm flex items-center gap-2">
                <Megaphone className="w-4 h-4 text-emerald-500" /> Donations & Sponsors
              </h3>
              <p className="text-xs text-text-secondary mt-1 mb-4">
                When enabled, a Support button appears in the footer linking to your donation/sponsor page (GitHub Sponsors, Ko-fi, Razorpay, etc.).
              </p>
              ...
              <form onSubmit={handleSaveDonation} className="space-y-4">
                <div className="flex items-center justify-between p-4 rounded-xl bg-bg-subtle border border-border">
                  <div>
                    <h4 className="text-sm font-bold text-text">Show Support button</h4>
                    <p className="text-xs text-text-secondary">Visible to all visitors in the footer bottom strip.</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => setDonationForm({ ...donationForm, donation_enabled: !donationForm.donation_enabled })}
                    aria-pressed={donationForm.donation_enabled}
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${donationForm.donation_enabled ? 'bg-emerald-500' : 'bg-bg-hover'}`}
                  >
                    <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${donationForm.donation_enabled ? 'translate-x-6' : 'translate-x-1'}`} />
                  </button>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-semibold text-text-secondary mb-1.5" htmlFor="donation-label">Button label</label>
                    <input
                      id="donation-label"
                      className="w-full bg-bg border border-border rounded p-2 text-sm text-text"
                      value={donationForm.donation_label}
                      onChange={(e) => setDonationForm({ ...donationForm, donation_label: e.target.value })}
                      maxLength={60}
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-text-secondary mb-1.5" htmlFor="donation-url">Donation / sponsor URL</label>
                    <input
                      id="donation-url"
                      type="url"
                      placeholder="https://github.com/sponsors/openlysts"
                      className="w-full bg-bg border border-border rounded p-2 text-sm text-text"
                      value={donationForm.donation_url}
                      onChange={(e) => setDonationForm({ ...donationForm, donation_url: e.target.value })}
                    />
                  </div>
                </div>
                ...
                <button
                  ...
                  className="px-4 py-2 rounded-lg bg-accent text-accent-fg text-xs font-bold hover:opacity-90 disabled:opacity-50 transition-all flex items-center gap-1.5 touch-target"
                >
                  {donationSaving && <Loader2 className="w-3.5 h-3.5 animate-spin" />}
                  {donationSaving ? 'Saving…' : 'Save donation settings'}
                </button>
              </form>
            </div>
            ...
            <div className="card p-5 sm:p-6">
              <h3 className="font-bold text-text text-sm flex items-center gap-2">
                <Coins className="w-4 h-4 text-amber-400" /> Sponsored Placements
              </h3>
              <p className="text-xs text-text-secondary mt-1 mb-4">
                Schedule a paid placement: the item shows a visible “{placementForm.badge_label || 'Sponsored'}” badge while the schedule is live. Placements never change organic ranking — disclosure is always shown.
              </p>

              {/* List */}
              {placements.length === 0 ? (
                <p className="text-xs text-text-muted bg-bg-subtle border border-dashed border-border rounded-xl p-4">
                  No placements yet. Create your first one below.
                </p>
              ) : (
                  ...
                  <table className="w-full text-left text-xs min-w-[640px]">
                    <thead>
                      <tr className="text-text-muted border-b border-border">
                        ...
                        <th className="py-2 pr-3 font-semibold">Sponsor</th>
                    ...
                    <tbody>
                      {placements.map((p) => (
                        <tr key={p.id} className="border-b border-border/40 align-top">
                          <td className="py-2.5 pr-3 font-semibold text-text max-w-[220px]">
                            <span className="block truncate" title={p.target_name}>{p.target_name}</span>
                          </td>
                          <td className="py-2.5 pr-3 text-text-secondary capitalize">{p.placement_type}</td>
                          <td className="py-2.5 pr-3">
                            <span className="inline-flex items-center gap-1 text-[9px] uppercase font-bold tracking-wider px-1.5 py-0.5 rounded border border-amber-400/40 bg-amber-400/10 text-amber-600 dark:text-amber-300">
                              {p.badge_label}
                            </span>
                          </td>
                          <td className="py-2.5 pr-3 text-text-secondary max-w-[160px]">
                            <span className="block truncate" title={p.sponsor_name || ''}>{p.sponsor_name || '—'}</span>
                          </td>
                          <td className="py-2.5 pr-3 text-text-secondary whitespace-nowrap">
                            ...
                            {' → '}
                            {p.ends_at ? new Date(p.ends_at).toLocaleDateString() : 'forever'}
                          </td>
                          <td className="py-2.5 pr-3">
                            <span className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-bold ${
                              p.status === 'live' ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-300' :
                              p.status === 'scheduled' ? 'bg-sky-500/10 text-sky-600 dark:text-sky-300' :
                              p.status === 'expired' ? 'bg-zinc-500/10 text-text-muted' :
                              'bg-amber-500/10 text-amber-600 dark:text-amber-300'
                            }`}>
                              {p.status === 'live' ? '● Live' : p.status === 'scheduled' ? '○ Scheduled' : p.status === 'expired' ? 'Expired' : 'Paused'}
                            </span>
                          </td>
                          <td className="py-2.5">
                            <div className="flex items-center gap-1.5">
                              <button
                                ...
                                className="p-1.5 rounded-lg text-text-muted hover:text-text hover:bg-bg-subtle transition-colors touch-target"
                                title="Edit placement"
                                aria-label={`Edit placement for ${p.target_name}`}
                              >
                                <Pencil className="w-3.5 h-3.5" />
                              </button>
                              <button
                                onClick={() => handleDeletePlacement(p)}
                                className="p-1.5 rounded-lg text-text-muted hover:text-trending hover:bg-bg-subtle transition-colors touch-target"
                                title="Delete placement"
                                aria-label={`Delete placement for ${p.target_name}`}
                              >
                                <Trash2 className="w-3.5 h-3.5" />
                              </button>
                            </div>
              ...
              {/* Add / Edit form */}
              {/* Schedule calendar */}
              <PlacementsCalendar placements={placements} />

              <form onSubmit={handleSavePlacement} className="space-y-4 border-t border-border pt-5">
                <h4 className="text-sm font-bold text-text flex items-center gap-2">
                  <CalendarClock className="w-4 h-4 text-accent" />
                  {editingPlacementId ? `Edit placement #${editingPlacementId}` : 'New sponsored placement'}
                  {editingPlacementId && (
                    <button
                      type="button"
                      onClick={() => { setEditingPlacementId(null); setPlacementForm(EMPTY_PLACEMENT); }}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-semibold text-text-secondary mb-1.5" htmlFor="pl-type">Placement type</label>
                    <select
                      id="pl-type"
                      className="w-full bg-bg border border-border rounded p-2 text-sm text-text"
                    ...
                    >
                      <option value="repository">Repository (project card)</option>
                      <option value="alternative">Alternative (SaaS tool)</option>
                    </select>
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-text-secondary mb-1.5" htmlFor="pl-target">Target name</label>
                    <input
                      id="pl-target"
                      required
                      list="sponsor-targets"
                      placeholder="e.g. open-webui/open-webui or Coolify"
                      className="w-full bg-bg border border-border rounded p-2 text-sm text-text"
                      value={placementForm.target_name}
                      onChange={(e) => setPlacementForm({ ...placementForm, target_name: e.target.value })}
                    />
                    <datalist id="sponsor-targets">
                      {sponsorSuggestions.map((n) => <option key={n} value={n} />)}
                    </datalist>
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-text-secondary mb-1.5" htmlFor="pl-badge">Badge label</label>
                    <input
                      id="pl-badge"
                      className="w-full bg-bg border border-border rounded p-2 text-sm text-text"
                      value={placementForm.badge_label}
                      onChange={(e) => setPlacementForm({ ...placementForm, badge_label: e.target.value })}
                      maxLength={24}
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-text-secondary mb-1.5" htmlFor="pl-sponsor">Sponsor name (shown in tooltip)</label>
                    <input
                      id="pl-sponsor"
                      className="w-full bg-bg border border-border rounded p-2 text-sm text-text"
                      value={placementForm.sponsor_name}
                      onChange={(e) => setPlacementForm({ ...placementForm, sponsor_name: e.target.value })}
                      maxLength={80}
                    />
                  </div>
                  <div className="sm:col-span-2">
                    <label className="block text-xs font-semibold text-text-secondary mb-1.5" htmlFor="pl-url">Sponsor URL (optional, tooltip link target)</label>
                    <input
                      id="pl-url"
                      type="url"
                      className="w-full bg-bg border border-border rounded p-2 text-sm text-text"
                      value={placementForm.sponsor_url}
                      onChange={(e) => setPlacementForm({ ...placementForm, sponsor_url: e.target.value })}
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-text-secondary mb-1.5" htmlFor="pl-start">Starts</label>
                    <input
                      id="pl-start"
                      type="datetime-local"
                      className="w-full bg-bg border border-border rounded p-2 text-sm text-text"
                      value={placementForm.starts_at}
                      onChange={(e) => setPlacementForm({ ...placementForm, starts_at: e.target.value })}
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-text-secondary mb-1.5" htmlFor="pl-end">Ends (empty = no end date)</label>
                    <input
                      id="pl-end"
                      type="datetime-local"
                      className="w-full bg-bg border border-border rounded p-2 text-sm text-text"
                      value={placementForm.ends_at}
                      onChange={(e) => setPlacementForm({ ...placementForm, ends_at: e.target.value })}
                    />
                  </div>
                </div>

                <div className="flex flex-wrap items-center gap-4">
                  <label className="flex items-center gap-2 text-xs font-semibold text-text-secondary cursor-pointer touch-target">
                    <input
                      type="checkbox"
                      checked={!!placementForm.active}
                      onChange={(e) => setPlacementForm({ ...placementForm, active: e.target.checked ? 1 : 0 })}
                  ...
                  <button
                    ...
                    className="px-4 py-2 rounded-lg bg-accent text-accent-fg text-xs font-bold hover:opacity-90 disabled:opacity-50 transition-all flex items-center gap-1.5 touch-target"
                  >
                    {placementSaving && <Loader2 className="w-3.5 h-3.5 animate-spin" />}
                    {placementSaving ? 'Saving…' : (editingPlacementId ? 'Update placement' : 'Create placement')}
                  </button>
                </div>
              </form>
            </div>
