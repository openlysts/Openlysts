...
const MODULES = [
  ...
  },
  {
    ...
    title: 'See Which Repo Replaces Which Paid Tool — Verified by Votes',
    badge: 'Crowd-Powered Discovery',
    icon: Share2,
    audience: 'Everyone',
    tagline: 'Every repository page now shows the community graph: evidence-backed “replaces X” links, pending suggestions, and vote buttons on every edge.',
    description: 'Behind Openlysts is a knowledge graph of alternatives. Each edge records the subject paid tool, the candidate open-source repo, and the evidence (a URL + title that back the claim). Anyone can suggest an edge, and anyone can vote it up or down — no moderator needed. An edge becomes Verified once it reaches a quorum of 3 distinct voters with a net +2 score; enough downvotes demote it back to Pending.',
    howItWorks: 'Verified = 3 distinct voters AND net ≥ +2 · Demoted = net ≤ −2 · 1 vote per IP max',
    highlights: [
      'Every repo page has a Community Graph card — empty state included, with an inline “Suggest an alternative” form.',
      'Vote buttons (↑/↓) on each edge; your IP can only count once per edge.',
      'Ideal for research: type a paid tool, get candidate repos with evidence links.',
      'No humans in the loop — the quorum sweeps automatically each ingestion cycle.'
    ],
    whoHelps: 'Find trustworthy replacements and help the community verify them.',
    ctaPath: '/alternatives',
    ctaText: 'Explore Alternatives'
  },
  {
    id: 'growth-engine',
    label: 'Self-Growing Catalog',
    title: 'The Catalog Grows Itself — Every Single Day',
    badge: 'Autonomous Living Index',
    icon: TrendingUp,
    audience: 'Everyone',
    tagline: 'A daily GitHub Actions job (the “window-cron”) runs quietly in the background and adds brand-new repos to the catalog so the number you see is always climbing.',
    description: 'The catalog counter you see on the Discover page isn\'t a fixed number — it\'s the live count of real repositories in Neon, including long-tail repos discovered through star-window enumeration. Each night, a bounded GitHub search cycle scans the long tail of the star spectrum and inserts genuinely new open-source projects, all within the free-tier budgets of GitHub, Neon, and Vercel.',
    howItWorks: 'Bounded cycle: windowDiscovery enumerates star windows → quotaGuard caps queries per run → inserted into LongTailRepo → counter climbs',
    highlights: [
      'The counter on /discover reflects the real live total — nothing hardcoded.',
      'The category strip shows honest counts: the curated repo categories and replacement niches you see are measured live.',
      'Zero maintenance: the workflow runs unattended on a schedule and stays inside free-tier limits.',
      'Your saved feeds and comparisons keep working while the catalog grows underneath.'
    ],
    whoHelps: 'See the index come alive — and trust that new discoveries keep flowing in.',
    ctaPath: '/discover',
    ctaText: 'Watch the Counter'
  },
  {
    id: 'why-openlysts',
    label: 'Why Openlysts',
    title: 'The Telescope for the Open-Source Universe',
    badge: 'The Mission',
    icon: Telescope,
    audience: 'Everyone',
    tagline: 'Not just another trending page — an evidence-ranked, community-verified discovery engine for the best living code on Earth.',
    description: 'Why Openlysts exists: over 300 million repositories exist, but most are abandoned experiments, tutorials, or star-farmed hype. Openlysts is the high-velocity telescope engineered to cut through the noise — surfacing living code, verified alternatives, and pure engineering craft in milliseconds. The “Why Openlysts” page carries the full manifesto, live stats, and the story behind the build.',
    howItWorks: 'Signal over noise: velocity & maintenance scores → no vanity stars · Community graph → verified alternatives · Free tier always → zero cost to use',
    highlights: [
      'The “Why Openlysts” page is one click away in the header nav and footer.',
      'Live rolling stats (repositories scanned, free alternatives) render from the real API.',
      'Anti-hype: repos are ranked by trajectory, not star count.'
    ],
    whoHelps: 'Understand the mission and philosophy behind the engine.',
    ctaPath: '/about',
...
const GOALS = [
  ...
  { id: 'community-graph', label: '🕸️ Verify which repo replaces which tool', tabId: 'community-graph' },
  { id: 'growth-engine', label: '📈 Watch the catalog grow itself', tabId: 'growth-engine' },
  { id: 'why-openlysts', label: '🔭 Why Openlysts exists', tabId: 'why-openlysts' }
];

export default function Guide() {
  const { totalRepositories, totalCategories, totalAlternatives } = usePlatformStats();
                ...
                <div className="space-y-4">
                  ...
                  <div className="relative rounded-xl overflow-hidden bg-black/90 aspect-video border border-border flex items-center justify-center group cursor-pointer">
                    ...
                    <div className="relative z-10 text-center p-4">
                      <div className="w-12 h-12 rounded-full bg-accent text-accent-fg flex items-center justify-center mx-auto mb-2 group-hover:-translate-y-0.5 transition-transform shadow-lg">
...
{`[
  {
    ...
    "saved_date": "2026-08-23"
              ...
              {/* Tab 8: Community Graph & Vote Quorum Simulator */}
              {activeModule.id === 'community-graph' && (
                <div className="space-y-4">
                  <div className="flex items-center justify-between border-b border-border pb-3">
                    <span className="text-xs font-bold text-text uppercase tracking-wider flex items-center gap-1.5">
                      <Share2 className="w-3.5 h-3.5 text-accent" />
                      Live Edge: zulip → Slack
                    </span>
                    <span className="text-[11px] px-2.5 py-0.5 rounded-full bg-accent-soft text-accent font-bold border border-accent/20">
                      Status: VERIFIED
                    </span>
                  </div>

                  <p className="text-xs text-text-secondary leading-relaxed">
                    This edge reached the 3-distinct-voter quorum and is now Verified — it ranks first on the
                    repo's Community Graph card. Hit the voting buttons to preview how the sweep works:
                  </p>

                  <div className="p-4 rounded-xl bg-bg-card border border-border space-y-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2.5">
                        <span className="w-8 h-8 rounded-full bg-zinc-200 dark:bg-white/10 flex items-center justify-center text-sm">⚡</span>
                        <div>
                          <div className="text-xs font-bold text-text">Zulip</div>
                          <div className="text-[11px] text-text-muted">Replaces Slack with open-source team chat</div>
                        </div>
                      </div>
                      <div className="text-[11px] text-right text-text-muted">
                        <div className="font-mono font-bold text-accent">↑ 5 · ↓ 1</div>
                        <div>net +4</div>
                      </div>
                    </div>
                    <div className="flex items-center gap-2 pt-3 border-t border-border">
                      ...
                      <span className="text-[11px] text-text-muted">
                        Evidence: github.com/RunaCapital/awesome-oss-alternatives
                      </span>
                    </div>
                  </div>

                  <div className="p-3 rounded-xl bg-bg-subtle border border-border text-[11px] text-text-secondary leading-relaxed">
                    <span className="font-bold text-text block mb-1">The quorum rule:</span>
                    An edge becomes Verified at <strong>3 distinct voters</strong> with net ≥ +2. Enough
                    downvotes (net ≤ −2) demote it back to Pending — and one IP can only count once per edge.
                  </div>
                </div>
              )}

              {/* Tab 9: Self-Growing Catalog Simulator */}
              {activeModule.id === 'growth-engine' && (
                <div className="space-y-5">
                  <div className="flex items-center justify-between border-b border-border pb-3">
                    <span className="text-xs font-bold text-text uppercase tracking-wider flex items-center gap-1.5">
                    ...
                    <span className="text-[11px] px-2.5 py-0.5 rounded-full bg-accent-soft text-accent font-bold border border-accent/20">
                      Runs Daily · 04:17 UTC
                    </span>
                  </div>

                  <p className="text-xs text-text-secondary leading-relaxed">
                    Every night a bounded GitHub search enumerates a fresh star window and inserts genuinely
                    new long-tail repos. The counter you see on Discover is the real, live total — it climbs
                    automatically:
                  </p>
                  ...
                  <div className="p-4 rounded-xl bg-bg-card border border-border space-y-3">
                    ...
                    <div className="w-full h-3 rounded-full bg-bg-subtle border border-border overflow-hidden">
                      <motion.div
                        className="h-full bg-gradient-to-r from-accent to-emerald-400 rounded-full"
                        initial={{ width: '62%' }}
                        animate={{ width: `${Math.min(100, 60 + (totalRepositories % 40))}%` }}
                        transition={{ duration: 1.2, ease: 'easeOut' }}
                      />
                    </div>
                    <div className="flex items-center justify-between text-[11px] text-text-muted pt-1">
                      <span>Last cycle: +a few hundred long-tail repos</span>
                      <span>Budget: within GitHub · Neon free tier</span>
                    </div>
                  </div>

                  <div className="p-3 rounded-xl bg-bg-subtle border border-border text-[11px] text-text-secondary leading-relaxed">
                    <span className="font-bold text-text block mb-1">Free-tier guardrails:</span>
                    Every cycle is capped by <strong>quotaGuard</strong> — bounded queries per run, Neon storage
                    measured with quoted identifiers, and no cycle can blow past the free limits.
                  </div>
                </div>
              )}

              {/* Tab 10: Why Openlysts / Mission Simulator */}
              {activeModule.id === 'why-openlysts' && (
                <div className="space-y-5">
                  <div className="flex items-center justify-between border-b border-border pb-3">
                    <span className="text-xs font-bold text-text uppercase tracking-wider flex items-center gap-1.5">
                      <Telescope className="w-3.5 h-3.5 text-accent" />
                      The Mission in Numbers (live)
                    </span>
                    <span className="text-[11px] px-2.5 py-0.5 rounded-full bg-accent-soft text-accent font-bold border border-accent/20">
                      Life Dashboard
                    </span>
                  ...
                  <p className="text-xs text-text-secondary leading-relaxed">
                    Openlysts is the telescope for the open-source universe — every stat below is fetched
                    live from the engine, every time:
                  </p>
                  ...
                  <div className="grid grid-cols-3 gap-2.5">
                    <div className="p-3 rounded-xl bg-bg-card border border-border text-center">
                      ...
                      <div className="text-[10px] text-text-muted font-semibold mt-0.5">Repositories</div>
                    </div>
                    <div className="p-3 rounded-xl bg-bg-card border border-border text-center">
                      <div className="text-lg font-black text-accent">
                        {totalCategories.toLocaleString()}
                      </div>
                      <div className="text-[10px] text-text-muted font-semibold mt-0.5">Replacement Niches</div>
                    </div>
                    <div className="p-3 rounded-xl bg-bg-card border border-border text-center">
                      <div className="text-lg font-black text-text">
                        {totalAlternatives.toLocaleString()}
                      </div>
                      <div className="text-[10px] text-text-muted font-semibold mt-0.5">Alternatives</div>
                    </div>
                  </div>

                  <div className="p-3 rounded-xl bg-bg-subtle border border-border text-[11px] text-text-secondary leading-relaxed">
                    <span className="font-bold text-text block mb-1">The anti-hype stance:</span>
                    Repos are ranked by velocity and maintenance — not vanity stars. The full manifesto,
                    live stats, and the creator story all live on the <strong>“Why Openlysts”</strong> page, one
                    click away in the header nav and footer.
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          ...
          >
            <div className="w-10 h-10 rounded-xl bg-accent-soft text-accent flex items-center justify-center mb-4 group-hover:-translate-y-0.5 transition-transform border border-accent/20">
          ...
          >
            ...
            <h3 className="text-base font-bold text-text mb-1 group-hover:text-accent transition-colors">Why Openlysts</h3>
            <p className="text-xs text-text-muted leading-relaxed">The telescope for the open-source universe — mission, manifesto, and live stats.</p>
          </Link>
        </div>
