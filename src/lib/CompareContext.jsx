...
export function CompareProvider({ children }) {
  ...
  // Clear the tray when the user signs out or deletes their account, so a
  // shared device never shows the previous user's comparison selection.
  useEffect(() => {
    const onClearLocalState = () => setSelectedForCompare([]);
    window.addEventListener('openlysts:clear-local-state', onClearLocalState);
    return () => window.removeEventListener('openlysts:clear-local-state', onClearLocalState);
  }, []);
