...
// three.js (~500 KB) is a decorative background — defer it until after first paint
// so it never blocks LCP, and skip it entirely for reduced-motion users.
import { lazy, Suspense } from 'react';
import { getSettings } from '@/lib/settings';
import { prefersReduced } from '@/lib/accessibility';
...
export default function OpenlystLayout() {
  ...
  // Defer the WebGL background past first paint; respect reduced-motion + user's
  // background preference ('none') so the ~500 KB three chunk is not fetched.
  const [showBackground, setShowBackground] = useState(false);
  useEffect(() => {
    if (typeof window === 'undefined') return
    if (prefersReduced(getSettings)) return
    if ((getSettings().backgroundType || 'particles') === 'none') return
    const t = setTimeout(() => setShowBackground(true), 5000)
  ...
  return (
    <div className="min-h-screen flex flex-col theme-transition relative">
      ...
      <div className="relative z-10 flex flex-col flex-1">
        ...
        {/* Wrapper only — Footer.jsx renders the real <footer> landmark */}
