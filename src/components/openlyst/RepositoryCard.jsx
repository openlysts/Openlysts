...
export default function RepositoryCard({ repo, index = 0, view = 'grid', showTrendingBadge = true }) {
  ...
  const placement = findPlacement(sponsorshipsData?.placements, 'repository', [
    repo?.full_name,
  ...
  const background = useMotionTemplate`radial-gradient(circle at ${gradientX}% ${gradientY}%, rgba(var(--accent-rgb, 100, 200, 100), 0.07) 0%, rgba(var(--accent-rgb, 100, 200, 100), 0.03) 35%, transparent 70%)`;
  ...
  // Only the first few cards animate in — in long grids the entrance springs
  // for dozens of below-fold cards burn the main thread for nothing.
  return (
    <motion.div
      initial={index < 8 ? { opacity: 0, y: 12 } : false}
          ...
          <div className={view === 'list' ? 'flex-1 min-w-0 md:pr-6 flex flex-col' : 'w-full flex-1 flex flex-col justify-start'}>
            ...
            <div className="min-w-0 flex-1">
              <div className="flex flex-wrap gap-1.5 mb-2 pointer-events-auto">
                {isWebsite ? (
                  <div className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-semibold tracking-wide border border-purple-500/30 bg-purple-500/10 text-purple-600 backdrop-blur-md w-fit">
                    <Globe className="w-3 h-3" />
                    Website
                  </div>
                ) : isOpenSourceProduct ? (
                  <div className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-semibold tracking-wide border border-emerald-500/30 bg-emerald-500/10 text-emerald-700 backdrop-blur-md w-fit shadow-[0_0_10px_rgba(16,185,129,0.15)]">
                ...
                {isAuthority && (
                  <div className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-semibold tracking-wide border border-blue-500/40 bg-blue-500/10 text-blue-600 backdrop-blur-md w-fit shadow-[0_0_8px_rgba(59,130,246,0.3)]">
                    <ShieldCheck className="w-3 h-3" />
                    Core
                  </div>
                )}
                {isEngaged && (
                  <div className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-semibold tracking-wide border border-green-500/40 bg-green-500/10 text-green-600 backdrop-blur-md w-fit shadow-[0_0_8px_rgba(34,197,94,0.3)]">
            ...
            <div className={`flex items-center flex-wrap gap-x-3 gap-y-1.5 ${view === 'list' ? 'md:justify-end' : ''}`}>
              ...
              {!isWebsite && repo?.html_url?.includes('github.com') && (
                <a
                  ...
                  className="flex items-center gap-1 text-[11px] font-bold px-2 py-1 rounded-md transition-all group/ingest overflow-hidden relative shadow-[0_0_8px_rgba(236,72,153,0.2)] hover:shadow-[0_0_12px_rgba(236,72,153,0.4)] pointer-events-auto border border-pink-500/20 hover:border-pink-500/40"
