...
export default function InfiniteDiscoveryFeed({ initialFilters = {}, onRefresh, isRefreshing }) {
  ...
  // When initial data arrives, populate feed. Only the first ~12 are rendered
  // (≈ 2x the visible columns); the rest load lazily via the infinite-scroll
  // trigger so the initial DOM stays light on long grids.
  useEffect(() => {
    if (initialData?.results) {
      setAllRepos(weightedShuffle(initialData.results).slice(0, 12));
            ...
            {allRepos.map((repo, index) => {
              ...
              return (
                <motion.div
                  key={repo.id}
                  layout
                  initial={index < 8 ? { opacity: 0, x: 20 } : false}
