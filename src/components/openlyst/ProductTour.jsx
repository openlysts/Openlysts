...
const CustomTooltip = ({
  ...
  return (
    <div
      {...tooltipProps}
      className="w-[min(90vw,360px)] sm:w-[400px] bg-bg-card dark:bg-[#1a1c23]/98 backdrop-blur-3xl border border-border/80 dark:border-white/10 rounded-[24px] shadow-[0_30px_60px_-15px_rgba(0,0,0,0.5),_0_0_40px_rgba(var(--accent-rgb),0.15)] overflow-hidden relative font-sans"
    >
      ...
      <div className="p-6">
        ...
        <div className={`mb-8 ${index === 0 ? 'text-center flex flex-col items-center mt-4' : 'text-left'}`}>
          {index === 0 && (
            <div className="w-16 h-16 bg-gradient-to-br from-accent to-trending rounded-[20px] flex items-center justify-center mb-6 text-white shadow-lg shadow-accent/20 rotate-3 border border-white/20 relative overflow-hidden group hover:-translate-y-1 transition-all duration-300">
              <Lightbulb size={32} className="drop-shadow-md group-hover:-translate-y-0.5 transition-transform duration-300" />
            </div>
          )}
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className={`text-sm font-bold px-6 py-2.5 rounded-xl transition-all ${
                index === 0 
                  ? 'bg-gradient-to-r from-accent to-trending text-white shadow-lg shadow-accent/30 hover:shadow-accent/50 hover:-translate-y-1 active:scale-95' 
...
const ProductTour = () => {
  ...
  useEffect(() => {
    ...
    // Only auto-trigger the initial tour on /discover (its steps target
    // discover elements; starting it on the Welcome page spotlights nothing).
