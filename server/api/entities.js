...
// Only the public catalog is readable anonymously. Every other registered
// entity (User, Goal, Task, Agent, IngestionRun, …) holds personal/internal
// data and must never be listable/filterable without a session — a bare
// GET /api/entities/User/list used to dump names, emails and roles.
const PUBLIC_READ_ENTITIES = new Set(['Repository', 'Alternative']);
const requireReadAccess = (entity) => {
  if (PUBLIC_READ_ENTITIES.has(entity)) return null;
  return [requireAuth, requireRole(ROLES.ADMIN)];
};
...
router.get('/:entity/:action', async (req, res, next) => {
  ...
  const readGuard = requireReadAccess(entity);
  if (readGuard) {
    for (const mw of readGuard) {
...
router.post('/:entity/:action', async (req, res, next) => {
  ...
  const isReadAction = ['list', 'filter'].includes(action);
  
  if (isMutating || (isReadAction && requireReadAccess(entity))) {
