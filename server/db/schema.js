export async function initSchema(db) {
  const createQueries = [
    ...
    // ─── Phase 1 monetization: sponsored/featured placements + app settings ─
  ...
  const alterQueries = [
    ...
    `ALTER TABLE "Repository" ADD COLUMN IF NOT EXISTS tier TEXT DEFAULT 'cold'`,
    `ALTER TABLE "Repository" ADD COLUMN IF NOT EXISTS openlysts_score REAL DEFAULT 0`,
    ...
    // Bookmarks span BOTH repo id spaces: curated "Repository" UUIDs and
    // "LongTailRepo" full_name keys. The original FK only allowed the former,
    // silently dropping every long-tail bookmark; drop it so any identifier
    // can be saved (the GET join resolves display fields from either table).
    `ALTER TABLE "Bookmark" DROP CONSTRAINT IF EXISTS "Bookmark_repository_id_fkey"`
  ];
  ...
  const indexQueries = [
    ...
    // ─── Phase 1: Performance indexes for catalog scaling ─────────
    `CREATE INDEX IF NOT EXISTS idx_repo_openlysts_score ON "Repository"(openlysts_score DESC);`,
    `CREATE INDEX IF NOT EXISTS idx_repo_tier ON "Repository"(tier);`,
    `CREATE INDEX IF NOT EXISTS idx_repo_language ON "Repository"(language);`,
    `CREATE INDEX IF NOT EXISTS idx_repo_quality_score ON "Repository"(quality_score DESC);`,
    `CREATE INDEX IF NOT EXISTS idx_repo_engagement ON "Repository"(engagement_score DESC);`,
    `CREATE INDEX IF NOT EXISTS idx_repo_authority ON "Repository"(authority_score DESC);`,
    `CREATE INDEX IF NOT EXISTS idx_repo_featured ON "Repository"(featured) WHERE featured = 1;`,
    `CREATE INDEX IF NOT EXISTS idx_repo_staff_pick ON "Repository"(staff_pick) WHERE staff_pick = 1;`,
    `CREATE INDEX IF NOT EXISTS idx_repo_hidden ON "Repository"(hidden);`,
    // Composite index for common query pattern: score + tier + hidden
    `CREATE INDEX IF NOT EXISTS idx_repo_score_tier_hidden ON "Repository"(openlysts_score DESC, tier, hidden);`,
    // Alternative performance indexes
    `CREATE INDEX IF NOT EXISTS idx_alt_category ON "Alternative"(category);`,
    `CREATE INDEX IF NOT EXISTS idx_alt_openlysts_score ON "Alternative"(openlysts_score DESC);`,
    // Monetization indexes
    `CREATE INDEX IF NOT EXISTS idx_sponsored_active ON "SponsoredPlacement"(active, ends_at);`,
    `CREATE INDEX IF NOT EXISTS idx_sponsored_type_target ON "SponsoredPlacement"(placement_type, lower(target_name));`,
    // Session cleanup index
    `CREATE INDEX IF NOT EXISTS idx_session_expire_cleanup ON "session"(expire) WHERE expire < NOW();`,
  ];
...
/**
 * Idempotent, lazily-invoked DDL for the Phase-1 monetization tables.
 * Called on first use from the admin/public sponsorship routes so schema
 * creation never needs a manual migration (and never runs on every request).
 */
export async function ensureMonetizationTables(db) {
  const queries = [
    `CREATE TABLE IF NOT EXISTS "SponsoredPlacement" (
      id SERIAL PRIMARY KEY,
      placement_type TEXT NOT NULL,
  ...
  for (const q of queries) {
    try {
      ...
      console.error('[DB] ensureMonetizationTables error:', e.message);
    }
  }
}
