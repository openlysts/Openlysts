  ...
  for (const chunk of chunks) {
    await Promise.all(chunk.map(async (fullName) => {
      try {
        ...
        const item = await fetchRepoWithFallback(owner, name, tokenResult?.token || process.env.GITHUB_TOKEN);
        if (item && item.full_name) {
          const res = await ingestRepoItem(item, 'Trending', repoMap, snapshotMap);
          if (res?.repoData) {
            res.repoData.source_type = 'github';
            res.repoData.is_rising = true;
            const stars = Number(item.stargazers_count || res.repoData.stars || 1);
            res.repoData.velocity_score = Math.min(100, Math.round(Math.log10(Math.max(1, stars)) * 22));
          }
