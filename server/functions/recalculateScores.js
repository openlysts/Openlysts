...
import { calculateVelocityScore } from '../services/qualityScorer.js';
...
export default async function recalculateScores(req, res) {
  try {
    ...
    const updates = repos.map((repo) => {
      ...
      // No MetricSnapshot history means "0 growth" is a data gap, not a measured
      // zero — rank such repos by the honest magnitude proxy instead.
      const trending = snaps.length > 0
        ? calculateTrendingScore(g24, g7, g30)
        : calculateVelocityScore({ stars: repo.stars || 0, forks: repo.forks || 0, stargazers_count: repo.stars || 0 });
