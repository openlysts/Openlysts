    ...
    get(sid, callback) {
      if (pgStore) {
        pgStore.get(sid, (err, sessionData) => {
          ...
          // If the PG read succeeds but finds nothing (e.g. a Neon blip or a
          // pruned row) while this process still holds a live copy, serve the
          // in-memory session rather than silently logging the user out.
          if (!sessionData) {
            return memStore.get(sid, callback);
          }
