...
export function getGoogleCallbackUrl(reqOrBase) {
  ...
  return `${(process.env.APP_URL || 'https://openlysts.dpdns.org').replace(/\/$/, '')}/api/auth/google/callback`;
}
...
export function getGithubCallbackUrl(reqOrBase) {
  ...
  return `${(process.env.APP_URL || 'https://openlysts.dpdns.org').replace(/\/$/, '')}/api/auth/github/callback`;
}
